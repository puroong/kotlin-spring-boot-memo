...
class MemoController @Autowired constructor(
    ...
    @PreAuthorize("hasAuthority('READ_ALL_MEMO') or hasAuthority('READ_MY_MEMO') and authentication.name == #username")
    fun getMyMemos(@RequestHeader(SecurityConstants.HEADER_STRING) authorization: String,
                   ...
                   @RequestParam(value = "limit", required = false) limit: Int?): Flux<MemoDto> {
        return memoService.getMemos(username, tag, limit, offset)
    }
